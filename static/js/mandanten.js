...
(function () {
  ...
  function laden() {
    return fetch("/mandanten", { headers: authHeaders() })
      ...
      .then(function (d) {
        ...
        // Gemerkten Mandanten wegwerfen, wenn er zu diesem Konto nicht passt -
        // z.B. weil sich vorher jemand anderes an diesem Rechner angemeldet hat.
        var gemerkt = aktiverMandant();
        if (gemerkt) {
          var erlaubt = !ZUSTAND.darf ? false : ZUSTAND.mandanten.some(
            function (m) { return m.firma_id === gemerkt; });
          if (!erlaubt) setzeMandant(null);
        }
  ...
  function showPage() {
    ...
    ["startPage","zeitnachweisPage","buchPage","lohnPage","fahrtenbuchPage","mitarbeiterPage","settingsPage","teamPage","vorgaengePage"]
