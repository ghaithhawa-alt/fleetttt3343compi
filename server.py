...
ALL_MODULES = ["fahrtenbuch", "zeitnachweis", "lohnberechnung", "buchhaltung",
               "teamzugaenge", "vorgaenge"]

# Arten von Vorgaengen. "frei" ist alles, was jemand von Hand eintraegt.
VORGANG_ARTEN = {
    "fahrer_kassieren": "Fahrer abkassieren",
    "kasseninventur":   "Kasseninventur",
    "frei":             "Aufgabe",
}
...
RECHTE = {
    "inhaber":     {"zeitnachweis", "lohn", "buchhaltung", "fahrtenbuch",
                    ...
                    "team", "mandanten", "vorgaenge"},
    "buchhaltung": {"zeitnachweis", "lohn", "buchhaltung",
                    "mitarbeiter", "vertraege", "vorgaenge"},
...
class Vorgang(SQLModel, table=True):
    """Ein Auftrag von einer Person an die anderen.

    Beispiel: Die Buchhaltung legt an "Fahrer A - 534,60 EUR abkassieren".
    Der Vorgang liegt im gemeinsamen Korb; wer aus dem Team Zeit hat, kassiert
    ab und hakt ihn mit einem Hinweis ab.

    schluessel dient nur den automatisch erzeugten Vorgaengen (z.B. der
    taeglichen Kasseninventur). Er verhindert, dass derselbe Eintrag zweimal
    entsteht. Von Hand angelegte Vorgaenge lassen ihn leer (NULL) - und leere
    Werte gelten in der Eindeutigkeitspruefung als verschieden.
    """
    __table_args__ = (UniqueConstraint("firma_id", "schluessel", name="uq_vorgang"),)
    id: Optional[int] = Field(default=None, primary_key=True)
    firma_id: int = Field(foreign_key="firma.id", index=True)
    art: str = Field(default="frei", index=True)
    titel: str = ""
    schluessel: Optional[str] = None

    # Um wen geht es? Der Name wird mitkopiert, damit die Liste lesbar bleibt,
    # auch wenn der Mitarbeiter spaeter ausscheidet.
    mitarbeiter_id: int = 0
    mitarbeiter_name: str = ""

    # Geld immer in Cent - mit Kommazahlen rechnet man kein Bargeld ab.
...
class VorgangEreignis(SQLModel, table=True):
    """Die Zeitleiste eines Vorgangs. Wird nur angehaengt, nie geaendert -
    damit spaeter nachvollziehbar bleibt, wer wann was gemeldet hat."""
    id: Optional[int] = Field(default=None, primary_key=True)
    vorgang_id: int = Field(foreign_key="vorgang.id", index=True)
    firma_id: int = Field(foreign_key="firma.id", index=True)
    user_id: int = 0
    user_name: str = ""
    typ: str = "kommentar"        # angelegt | erledigt | kommentar | geoeffnet
    text: str = ""
    betrag_cent: int = 0
...
def get_wirk_user(x_mandant: Optional[int] = Header(default=None, alias="X-Mandant"),
    ...
    # Wer gar keine Mandanten fuehren darf, kann diesen Kopf nie absichtlich
    # geschickt haben - er stammt dann aus dem Browser eines Kollegen, der sich
    # vorher an demselben Rechner angemeldet hatte (fc_mandant im Speicher).
    # Ihn einfach zu ignorieren ist sicher: Der Benutzer landet in SEINER
    # eigenen Firma, bekommt also weniger zu sehen, nicht mehr. Wuerde hier
    # stattdessen ein Fehler kommen, waere die halbe App fuer ihn tot.
...
def _vorgaenge_modul_nachtragen():
    ...
    Laeuft nur so lange, wie es Firmen ohne das Modul gibt. Wer es bewusst
    entfernt, bekommt es beim naechsten Start allerdings wieder - das ist bei
    einem Grundmodul gewollt. Soll es abschaltbar sein, gehoert das in die
    Tarif-Tabelle, nicht hierher.
    """
    with Session(engine) as s:
        geaendert = 0
        for firma in s.exec(select(Firma)).all():
            module = json.loads(firma.modules_json or DEFAULT_MODULES)
            if "vorgaenge" in module:
                continue
            module.append("vorgaenge")
            firma.modules_json = json.dumps(module)
            s.add(firma)
            geaendert += 1
        if geaendert:
            s.commit()
            print(f"  Nachgetragen: Vorgaenge fuer {geaendert} Firma(en)", flush=True)
...
def _plan_grenzen_anwenden(firma: Firma, plan_key: str):
    """Setzt Platzgrenze und die tarifabhaengigen Module."""
    ...
    # Vorgaenge gehoeren zur Grundausstattung - die taegliche Kasseninventur
    # braucht jeder Betrieb, unabhaengig vom Tarif.
    if "vorgaenge" not in module:
        module.append("vorgaenge")
    firma.modules_json = json.dumps(module)
...
def admin_firmen(current: User = Depends(get_current_user),
    ...
    for u in users:
        ...
        if u.aktiv:
            aktive_je_firma[u.firma_id] = aktive_je_firma.get(u.firma_id, 0) + 1
    aktivitaet = {}
    ...
    return {"firmen": [{
        ...
        "benutzer_aktiv": aktive_je_firma.get(f.id, 0),
        "benutzer_gesamt": len(emails_by_firma.get(f.id, [])),
...
# ───────────────────────── Vorgänge ───────────────────────────────────────────
# Übergaben zwischen Kollegen: Die Buchhaltung sagt, was zu tun ist, die
# Disposition erledigt es und meldet zurück. Jeder Schritt landet in einer
# Zeitleiste, die nur wächst und nie überschrieben wird.

def _cent(betrag) -> int:
    """Euro-Eingabe in Cent.

    Muss mit allem umgehen, was Leute tippen: 534,60 · 534.60 · 1.234,56 ·
    1,234.56 · 1.234 (Tausender) · 12 · "534,60 €".

    Regel: Das LETZTE Komma oder der letzte Punkt ist das Dezimalzeichen -
    ausser es stehen genau drei Ziffern dahinter und es ist das einzige
    Trennzeichen. Dann ist es eine Tausendertrennung (1.234 = 1234 Euro).
    """
    if betrag is None or betrag == "":
        return 0
    if isinstance(betrag, (int, float)):
        return int(round(float(betrag) * 100))

    txt = str(betrag).strip().replace("€", "").replace(" ", "").replace(" ", "")
    if not txt:
        return 0
    minus = txt.startswith("-")
    txt = txt.lstrip("+-")
    # Erlaubte Schreibweisen - alles andere ist ein Tippfehler und wird
    # abgelehnt, statt still eine falsche Summe zu ergeben.
    if not re.fullmatch(
            r"\d+(?:[.,]\d{1,2})?"                 # 12 · 12,5 · 12,50 · 12.50
            r"|\d{1,3}(?:\.\d{3})+(?:,\d{1,2})?"   # 1.234 · 1.234,56
            # Englische Tausendertrennung nur MIT Nachkommastellen: "12,345"
            # waere sonst mehrdeutig (12,35 EUR oder 12.345 EUR?) - bei Geld
            # lieber ablehnen als raten.
            r"|\d{1,3}(?:,\d{3})+\.\d{1,2}",       # 1,234.56
            txt):
        raise HTTPException(status_code=400, detail=f"Kein gültiger Betrag: {betrag}")

    trenner = max(txt.rfind(","), txt.rfind("."))
    if trenner == -1:
        ganz, rest = txt, ""
    else:
        nachkomma = len(txt) - trenner - 1
        nur_einer = (txt.count(",") + txt.count(".")) == 1
        if nachkomma == 3 and nur_einer:
            ganz, rest = txt.replace(",", "").replace(".", ""), ""   # 1.234 = Tausender
        else:
            ganz = txt[:trenner].replace(",", "").replace(".", "")
            rest = txt[trenner + 1:]
    if rest and not rest.isdigit():
        raise HTTPException(status_code=400, detail=f"Kein gültiger Betrag: {betrag}")
    rest = (rest + "00")[:2]            # 12,5 -> 50 Cent, 12,567 -> 56 Cent
    try:
        wert = int(ganz or "0") * 100 + int(rest)
    except ValueError:
        raise HTTPException(status_code=400, detail=f"Kein gültiger Betrag: {betrag}")
...
def _vorgang_firma(current: User, session: Session, schreibend: bool = True) -> Firma:
    """Prüft Recht und Tarif. schreibend=False lässt auch "nur lesen" durch."""
    if schreibend:
        erlaubt = hat_recht(current, "vorgaenge")
    else:
        erlaubt = hat_recht(current, "vorgaenge") or hat_recht(current, "vorgaenge_lesen")
    if not erlaubt:
        raise HTTPException(status_code=403,
                            detail="Für Vorgänge fehlt dir die Berechtigung.")
    firma = session.get(Firma, current.firma_id)
    if firma is None:
        raise HTTPException(status_code=404, detail="Firma nicht gefunden")
    if not _is_superadmin(current) and "vorgaenge" not in json.loads(
            firma.modules_json or DEFAULT_MODULES):
        raise HTTPException(status_code=403,
                            detail="Vorgänge sind in deinem Tarif nicht enthalten.")
    return firma
...
def _ereignis(session: Session, vorgang: Vorgang, user: User, typ: str,
              text: str = "", betrag_cent: int = 0):
    session.add(VorgangEreignis(
        vorgang_id=vorgang.id, firma_id=vorgang.firma_id,
        user_id=user.id or 0, user_name=_wer(user),
        typ=typ, text=(text or "")[:1000], betrag_cent=betrag_cent))


def _vorgang_dict(v: Vorgang, ereignisse: Optional[list] = None) -> dict:
    d = {
        "id": v.id,
        "art": v.art,
        "art_name": VORGANG_ARTEN.get(v.art, v.art),
        "titel": v.titel or VORGANG_ARTEN.get(v.art, "Aufgabe"),
        "mitarbeiter_id": v.mitarbeiter_id or 0,
        "mitarbeiter_name": v.mitarbeiter_name or "",
        "betrag_soll": _euro(v.betrag_soll_cent),
        ...
        "erstellt_von_name": v.erstellt_von_name or "",
    ...
    if ereignisse is not None:
        d["verlauf"] = [{
            "typ": e.typ, "text": e.text or "", "wer": e.user_name or "",
            "betrag": _euro(e.betrag_cent) if e.betrag_cent else "",
            "wann": e.zeitpunkt.strftime("%d.%m.%Y %H:%M") if e.zeitpunkt else "",
        } for e in ereignisse]
    return d
...
def _kasseninventur_sicherstellen(session: Session, firma: Firma):
    ...
    Bewusst hier und nicht in einem Hintergrunddienst: So braucht Railway
    keinen zusaetzlichen Prozess, es gibt keine Zeitzonen-Ueberraschungen, und
    es funktioniert auch, wenn der Server zwischendurch geschlafen hat.
    Nicht abgehakte Tage bleiben offen stehen - genau das soll man sehen.
    """
    heute = date.today()
    schluessel = f"kasseninventur:{heute.isoformat()}"
    da = session.exec(select(Vorgang).where(Vorgang.firma_id == firma.id,
                                            Vorgang.schluessel == schluessel)).first()
    if da:
        return
    v = Vorgang(firma_id=firma.id, art="kasseninventur", schluessel=schluessel,
                titel=f"Kasseninventur {heute.strftime('%d.%m.%Y')}",
                faellig_am=heute, erstellt_von_name="automatisch")
    session.add(v)
    try:
        session.commit()
        session.refresh(v)
    except Exception:
        session.rollback()      # zwei Arbeiter gleichzeitig - einer gewinnt, gut so
        return
    session.add(VorgangEreignis(vorgang_id=v.id, firma_id=firma.id,
                                user_name="automatisch", typ="angelegt",
                                text="Täglicher Eintrag"))
    session.commit()
...
@app.get("/vorgaenge/anzahl")
def vorgaenge_anzahl(current: User = Depends(get_wirk_user),
                     session: Session = Depends(get_session)):
    """Nur die Zahl fürs Abzeichen am Menüpunkt - klein und oft abgefragt."""
    try:
        firma = _vorgang_firma(current, session, schreibend=False)
    except HTTPException:
        return {"offen": 0, "aktiv": False}
    offen = session.exec(select(Vorgang).where(Vorgang.firma_id == firma.id,
                                               Vorgang.status == "offen")).all()
    return {"offen": len(offen), "aktiv": True,
            "ueberfaellig": len([v for v in offen if v.faellig_am
                                 and v.faellig_am < date.today()])}


class VorgangNeuRequest(BaseModel):
    art: str = "fahrer_kassieren"
    titel: Optional[str] = None
    mitarbeiter_id: Optional[int] = None
    betrag: Optional[str] = None       # "534,60" oder 534.6
    faellig_am: Optional[str] = None   # YYYY-MM-DD
    hinweis: Optional[str] = None


@app.post("/vorgaenge")
def vorgang_anlegen(data: VorgangNeuRequest,
                    current: User = Depends(get_wirk_user),
                    session: Session = Depends(get_session)):
    firma = _vorgang_firma(current, session)
    art = (data.art or "frei").strip().lower()
    if art not in VORGANG_ARTEN:
        raise HTTPException(status_code=400, detail=f"Unbekannte Art: {data.art}")

    ma_id, ma_name = 0, ""
    if data.mitarbeiter_id:
        m = session.get(Mitarbeiter, int(data.mitarbeiter_id))
        if m is None or m.firma_id != firma.id:
            raise HTTPException(status_code=404, detail="Mitarbeiter nicht gefunden")
        ma_id, ma_name = m.id, m.name

    if art == "fahrer_kassieren" and not ma_id:
        raise HTTPException(status_code=400,
    ...
    session.add(v); session.commit(); session.refresh(v)
    _ereignis(session, v, current, "angelegt",
              (data.hinweis or "").strip(), v.betrag_soll_cent)
    session.commit()
    return _vorgang_dict(v)


class VorgangErledigtRequest(BaseModel):
...
def vorgang_erledigt(vorgang_id: int, data: VorgangErledigtRequest,
                     current: User = Depends(get_wirk_user),
                     session: Session = Depends(get_session)):
    """Abhaken: komplett oder nicht komplett, dazu der tatsächliche Betrag
    und ein Hinweis für die Buchhaltung."""
    firma = _vorgang_firma(current, session)
    ...
    if v.status != "offen":
        raise HTTPException(status_code=409, detail="Dieser Vorgang ist schon erledigt.")

    ergebnis = (data.ergebnis or "komplett").strip().lower()
    if ergebnis not in ("komplett", "teilweise"):
        raise HTTPException(status_code=400,
                            detail="Ergebnis muss 'komplett' oder 'teilweise' sein")
    hinweis = (data.hinweis or "").strip()
    if ergebnis == "teilweise" and not hinweis:
        raise HTTPException(status_code=400,
...
@app.post("/vorgaenge/{vorgang_id}/kommentar")
def vorgang_kommentar(vorgang_id: int, data: VorgangTextRequest,
                      current: User = Depends(get_wirk_user),
                      session: Session = Depends(get_session)):
    firma = _vorgang_firma(current, session)
    v = session.get(Vorgang, vorgang_id)
    ...
    if not text:
        raise HTTPException(status_code=400, detail="Der Hinweis ist leer.")
    _ereignis(session, v, current, "kommentar", text)
    session.commit()
    return {"ok": True}


@app.post("/vorgaenge/{vorgang_id}/wieder-oeffnen")
def vorgang_wieder_oeffnen(vorgang_id: int, data: VorgangTextRequest,
                           current: User = Depends(get_wirk_user),
                           session: Session = Depends(get_session)):
    """Falsch abgehakt? Der Vorgang wird wieder geöffnet - die alte Meldung
    bleibt aber in der Zeitleiste stehen und wird nicht überschrieben."""
    firma = _vorgang_firma(current, session)
    v = session.get(Vorgang, vorgang_id)
    if v is None or v.firma_id != firma.id:
        raise HTTPException(status_code=404, detail="Vorgang nicht gefunden")
    if v.status == "offen":
        raise HTTPException(status_code=409, detail="Dieser Vorgang ist bereits offen.")
    grund = (data.text or "").strip()
    if not grund:
        raise HTTPException(status_code=400, detail="Bitte einen Grund angeben.")
    v.status = "offen"
    v.ergebnis = ""
    v.erledigt_von = 0
    v.erledigt_von_name = ""
...
@app.get("/vorgaenge/{vorgang_id}")
def vorgang_detail(vorgang_id: int,
                   current: User = Depends(get_wirk_user),
                   session: Session = Depends(get_session)):
    firma = _vorgang_firma(current, session, schreibend=False)
    v = session.get(Vorgang, vorgang_id)
    if v is None or v.firma_id != firma.id:
        raise HTTPException(status_code=404, detail="Vorgang nicht gefunden")
    ereignisse = session.exec(
        select(VorgangEreignis).where(VorgangEreignis.vorgang_id == v.id)
        .order_by(VorgangEreignis.id)).all()
    return _vorgang_dict(v, ereignisse)
